...
STORE_URI="${CLAWGRAPH_STORE_URI:-sqlite:////tmp/clawgraph-agent-diff-logits-demo.db}"
PAYLOAD_DIR="${CLAWGRAPH_PAYLOAD_DIR:-/tmp/clawgraph-agent-diff-logits-payloads}"
PROXY_HOST="${CLAWGRAPH_PROXY_HOST:-127.0.0.1}"
PROXY_PORT="${CLAWGRAPH_PROXY_PORT:-8094}"
...
if ! curl -fsS "${AGENT_DIFF_HEALTH_URL}" >/dev/null 2>&1; then
  echo "agent-diff backend is not healthy on ${AGENT_DIFF_BASE_URL}" >&2
  echo "Start agent-diff backend first, then rerun this script." >&2
  exit 1
fi
...
if ! curl -fsS "${PROXY_BASE_URL%/v1}/health" >/dev/null 2>&1; then
  ...
  ./.venv/bin/clawgraph proxy \
    ...
    --host "${PROXY_HOST}" \
    --port "${PROXY_PORT}" \
    >/tmp/clawgraph-agent-diff-logits-proxy.log 2>&1 &
  PROXY_PID=$!
...
if ! curl -fsS "${PROXY_BASE_URL%/v1}/health" >/dev/null 2>&1; then
  echo "clawgraph proxy did not become healthy on ${PROXY_BASE_URL}" >&2
  exit 1
fi
...
./.venv/bin/python benchmarks/agent_diff/run_agent_diff_logits_pipeline.py "$@"
