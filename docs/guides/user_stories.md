...
4. Treat `session` as the container and `run` as one execution episode inside it.
5. Use `clawgraph inspect session --session latest`, `clawgraph list runs --session latest`, `clawgraph list requests --session latest`, and `clawgraph replay --session latest`.
6. Add stable ids and semantic events only where replay grouping or branch fidelity needs to improve.
...
2. Discover sessions, then select the run you want to export with `clawgraph list sessions` and `clawgraph list runs --session <id>`.
...
2. Resolve a target with `latest-response`, `latest-failed-branch`, or `run:latest`.
...
- which recent runs are export-ready and which sessions they belong to
...
2. Use `clawgraph list readiness --builder preference` to scan the most recent runs first.
