  ...
  if (!evalSuites.length && !scorecards.length) {
    return (
      <div className="space-y-6">
        <PageHeader
          title="评测"
          description="当前真实数据源里还没有评测套件或评分卡。先生成评测批次，再回来查看替代验证结果。"
          primaryAction={<Button href="/flows/validate-slice" variant="primary">查看验证流程</Button>}
          secondaryAction={<Button href="/coverage" variant="secondary">打开上线控制面</Button>}
        />
        <EmptyState
          actionHref="/flows/validate-slice"
          actionLabel="创建评测流程"
          description="当 store 中出现评测套件或评分卡后，这里会自动切换成可交互的评分工作区。"
          title="还没有评测资产"
        />
      </div>
    );
